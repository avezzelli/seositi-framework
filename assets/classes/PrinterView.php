...
class PrinterView
{
    ...
    private function getFloatLayout(string $type, string $name, string $label, string $required = '', ?string $value = null, string $disabled = ''): string
    {
        ...
        if ($type === 'price') {
            ...
            $input .= '<label style="display:block" for="' . $attrName . '" class="form-label">' . $attrLabel . '</label>'
                . '<input class="input form-control form-control-lg" id="' . $attrName . '" value="' . $attrValue . '" name="' . $attrName . '" type="file" ' . $attrReq . ' ' . $attrDis . '>';
        } else if ($type === 'image' || $type === 'immagine') {
            $immagine = '';
            if ($value !== null && $value !== '') {
                $immagine = '<img src="' . esc_url($value) . '" style="max-width:150px;">';
                $input .= '<input class="input-hidden" type="hidden" name="' . $attrName . '-old" value="' . esc_url($value) . '" />';
            }
            $input .= '<p><strong>' . $attrLabel . '</strong></p>'
                . '<div class="container-immagine">'
                . '<div style="display:inline-block; width:65%"><input class="input form-control form-control-lg" id="' . $attrName . '" value="' . $attrValue . '" name="' . $attrName . '" type="file" ' . $attrReq . ' ' . $attrDis . '></div>'
            ...
            $input .= $editorContent;
            $input .= '<label for="' . $attrName . '">' . $attrLabel . '</label>';
        }
